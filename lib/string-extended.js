...
String.prototype.inverseCase = function() {
	var stringCharacters = this.split('');
	for (var index = 0; index < stringCharacters.length; index++) {
		if (/[a-z]/g.test(stringCharacters[index])) {
			stringCharacters[index] = stringCharacters[index].toUpper();
			continue;
		}
		if (/[A-Z]/g.test(stringCharacters[index])) {
			stringCharacters[index] = stringCharacters[index].toLower();
			continue;
		}
	}
